...
C_CheckAllMsgAction::C_CheckAllMsgAction(T_CmdAction        P_cmdAction,
                                         T_pControllers P_controllers)
  : C_CommandAction (P_cmdAction, P_controllers) {
}

C_CheckAllMsgAction::~C_CheckAllMsgAction() {
}


T_exeCode    C_CheckAllMsgAction::execute(T_pCmd_scenario P_pCmd,
  ...
  if (L_check_result == false) {
    ...
    if (m_controllers.m_check_behaviour == E_CHECK_BEHAVIOUR_ERROR) {
